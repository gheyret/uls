 ...
 * Usage: document.addEventListener('DOMContentLoaded', Uls.init);
 */
class Uls {
    static map = {
        UL: {
    ...
    static convert(text, mode) {
        if (mode === 'ul') {
            return Uls.convertU2L(text);
        } else if (mode === 'us') {
            return Uls.convertU2S(text);
        } else {
            return text;
        }
    }
        ...
        for (const herp of token) {
            if (['\u0626', '\uFE8B', '\uFE8C'].includes(herp)) {
                ...
                if (mapped !== undefined) {
                    ...
                    const belge = Uls.convertU2Belge(herp);
                    result.push(belge);
                    skip = true;
                }
            }
    ...
    static convertAllTextNodes(node, mode) {
        if (node.nodeType === Node.TEXT_NODE && node.textContent.trim()) {
            node.textContent = Uls.convert(node.textContent, mode);
        } else {
    ...
    static init() {
        ...
        if (!document.querySelector('.uls-switcher-notice')) {
            ...
            div.style.cssText = 'background:#f0f8ff;padding:3px;border-radius:4px;margin:10px 0;font-family:"UKIJ Tuz","UKIJ Tor", "Tahoma" ;font-size:18px;text-align:left;direction:ltr';
            div.innerHTML = `
